...
import { formatPropertyValue, PROPERTY_DEFINITIONS } from '../utils/propertyFormatters';
...
export default function InverseDesign() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-white">Inverse Design</h1>
        ...
        <section className="glass-panel rounded-2xl p-4">
          ...
          {results.length > 0 ? (
              ...
              {results.map((candidate, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mono-smiles mt-3 break-all text-xs text-indigo-200">{candidate.smiles}</p>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-lg bg-black/20 p-2">
                        <div className="text-slate-500">{definition.label}</div>
                        <div className="mono-smiles text-white">
                      ...
                      <div className="rounded-lg bg-black/20 p-2">
                        ...
                        <div className="mono-smiles text-white">{candidate.descriptors.MolWt}</div>
                      </div>
                    </div>
                  </button>
                );
            ...
            <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-8 text-center text-sm text-slate-400">
              Candidate molecules will appear after a target search.
            </div>
          )}
        </section>
      </div>
