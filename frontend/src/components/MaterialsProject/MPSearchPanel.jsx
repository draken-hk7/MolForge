...
/**
 * Render Materials Project formula and element search.
...
export default function MPSearchPanel({ initialFormula = 'SiO2', onSelectMaterial = () => {} }) {
  const { mpMaterials, searchByFormula, searchByElements, getMaterialDetail, isLoading, error } = useMaterialsProject();
  ...
  return (
    <section className="glass-panel rounded-2xl p-4">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400">Search real materials by formula or element system</p>
      </div>
      <form onSubmit={handleSearch} className="space-y-3">
        <div className="flex rounded-lg border border-white/10 bg-black/20 p-1">
          {['formula', 'elements'].map((item) => (
            <button
              key={item}
              type="button"
              onClick={() => setMode(item)}
              className={cn('flex-1 rounded-md px-3 py-1.5 text-xs font-semibold capitalize', mode === item ? 'bg-blue-500 text-white' : 'text-slate-400')}
            >
              {item}
            </button>
          ))}
